...
const ProductCard = (props) => {
  ...
  // const prod_item_stock = props.product ? props.product.quantity : null;
  const [formData, setFormData] = useState(null);

  // console.log("prod_item_stock", prod_item_stock);
    ...
    if (cartItemQuantity == undefined) {
      if (cartItem > 0) {
        ...
        if (formData != null) {
          ...
          console.log("PRODUCT IDDDDDDDDD ------", data);
          // setFormData({
          //   id: product_id,
          //   quantity: cartItem,
          //   price: product_detail.price_euro,
  ...
  const decrementQuantity = () => {
    ...
    if (cartItemQuantity != undefined) {
      if (cartItemQuantity > 1) {
        // disptach(decrementItemQuantity({ id: product_id, quantity: cartItemQuantity - 1 }))
        disptach(
          decrementItemQuantity({
            id: product_id,
      ...
      >
        ...
        >
          ...
          {/* {Array.from({ length: 5 }, (i) => (
              <img src="/images/product_star.png" height={15} key={i} />
            ))} */}
        </div>
        <div className="prod_cart_option d-flex justify-content-between mb-3 pb-2">
          <div className="buttons_opt">
            ...
            <span className="cart_item">
              {/* {cartItemQuantity != undefined ? cartItemQuantity : cartItem} */}
              {cartItem}
            </span>
          ...
          {/* <div className="cart_btn_div">
            <span
              className={`btn cart_btn ${
                prod_item_stock != null &&
                prod_item_stock <= 0 &&
                "out_of_stock"
              }`}
              onClick={() => addedState()}
            >
              {prod_item_stock != null && prod_item_stock > 0
                ? trans("Add To Cart")
                : trans("Out of Stock")}
            </span>
          </div> */}
...
export default translate(ProductCard);
