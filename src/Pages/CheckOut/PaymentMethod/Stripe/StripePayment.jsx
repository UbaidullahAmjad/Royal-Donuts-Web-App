...
const StripePayment = (props) => {
  ...
  console.log("stripeInvoiceData", stripeInvoiceData)

  useEffect(() => {
    if (stripeResponseError == false && stripeInvoiceData != null) {
      console.log("stripeInvoiceData-useEffect is called")
      ...
      props.LoadingInvoice(false);
      console.log("stripeInvoiceData-useEffect stripeInvoiceData is null")
    }
  }, [stripeInvoiceData])
  ...
  const handleSubmit = async () => {
    ...
    if (formDataPushed?.user_id == null) {
      ...
      console.log('stripeEmailCheckResp***1', StripeEmailCheckActionCall);

      if (StripeEmailCheckActionCall?.data?.success == true) {
        console.log('stripeEmailCheckResp***', StripeEmailCheckActionCall);

        PaymentWithInvoice();
        dispatch(ClearStripeEmailCheckAction())
      } else {
        console.log('stripeEmailCheckResp-error***', StripeEmailCheckActionCall);
        props.LoadingInvoice(false);
  ...
  const PaymentWithInvoice = () => {
    StripePayment().then((stripe_response) => {
      console.log("STRIPE_DATA 2222 ----------", stripe_response);
      if (stripe_response?.error) {
        ...
        console.log("stripe-all-data", allData);
