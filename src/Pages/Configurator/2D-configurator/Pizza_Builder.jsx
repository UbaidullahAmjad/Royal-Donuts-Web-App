...
const Pizza_Builder = (props) => {
  ...
  const setSauce = (data) => {
    console.log("SAUCESSS DATA -----------------", data);
  ...
  const setFilling = (data) => {
    console.log("FILLIN DATA -----------------", data);
    console.log("SELECTED FILLIN DATA -----------------", selectedFilling);
    if (selectedFilling.length > 0) {
      if (data[0].name == selectedFilling[0].name) {
        setSelectedFilling([]);
        setFillingPrice(0);
      } else {
        console.log("FILLIN DATA -----------------", data[0].unselect);
        if (data[0].unselect == undefined) {
  ...
  const placeOrder = () => {
    ...
    // throw new Error("CONFIRTMED ORDER ERROR");
    if (
      ...
      // confirmedOrder?.topping.sort((a, b) => a.name.localeCompare(b.name));
      console.log("CONFIRTMED ORDER --------------------", confirmedOrder);
      const get_donut_item = DonutComparison(
        get_customized_donut_array,
        confirmedOrder
      );
      console.log(" GET DONUT ITEMS  -----------", get_donut_item);
...
export default translate(Pizza_Builder);
