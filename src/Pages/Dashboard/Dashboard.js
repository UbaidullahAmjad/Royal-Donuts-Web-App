...
const Dashboard = (props) => {
  ...
  // console.log("DashboardUserData-getDashboardData", getDashboardData)
  // console.log("DashboardUserData", dashboard_user)
  // console.log("DashboardUserData-deliveredOrders", deliveredOrders)
  // console.log("DashboardUserData-OpenOrdersList", openOrders)

  useEffect(() => {
    if (dashboard_user == null || (dashboard_user?.id != undefined && dashboard_user?.id != original_id)) {
      dispatch(DashboardDataAction(original_id))
      // console.log("DashboardUserData-DashboardDataAction-useEffect is called")
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (getDashboardData.dashboardDataLength != getDashboardData.tempArrLength) {
      dispatch(DashboardDataAction(original_id))
      // console.log("DashboardUserData-DashboardDataAction-useEffect-2 is called")
    }

    // console.log("DashboardUserData-DashboardData useEffect2 is called")
    // console.log("DashboardUserData-DashboardData useEffect2:", dashboard_user)
  ...
  const handleOpen = (row) => {
    ...
    axios(config)
      .then(function (response) {
        console.log("response-open_orders", response);
  ...
  const columnsOpenOrders = [
    ...
    },
    {
      ...
      renderCell: (cellValues) => (
        <>
          {/* {moment(
            cellValues.row?.delivery_date?.toString(),
            "DD/MM/YYYY hh:mm:ss"
          ).format("ddd, Do MMM, YYYY HH:mm:ss") != "Invalid date"
            ? moment(
                cellValues.row?.delivery_date?.toString(),
                "DD/MM/YYYY hh:mm:ss"
                      ...
                      <div className="row m-0">
                        <div className="col-md-6 ps-0 pe-0 pe-md-1">
                          ...
                          >
                            ...
                            // {...register('firstName', { required: true, maxLength: 20, pattern: /^[a-zA-Z0-9.\s]+$/ })}
                            />
                            <span
