...
const instagram = (props) => {
  ...
  useEffect(() => {
    const instagram_media_url = `https://graph.instagram.com/me/media?fields=id,media_type,media_url,caption&limit=${6}&access_token=${instagramToken}`;
    axios.get(instagram_media_url).then((response) => {
      console.log("instagram-media-response", response)
  ...
  console.log("token", instaValidToken);
  return (
    <>
      ...
      {
        InstagramMediaImages && InstagramMediaImages.length > 0 &&
        InstagramMediaImages.map((item) => <img style={{ width: 150, }} id={item.id} src={item.media_url} />)
      }
    </>
  );
};
