...
const main = (props) => {
  ...
  useEffect(() => {
    ...
    console.log("categoriesList.categoriesList", categoriesList.categoriesList);
    ...
    console.log("productsList.productsList ", productsList.productsList);
    if (productsList.productsList != null) {
      // Products List
      const products = productsList.productsList.products;
      setCategoryAllProducts(products);
  ...
  console.log("categoryAllProducts --- ", categoryAllProducts);
  ...
  console.log("cateProdLoading", cateProdLoading);
  ...
  const getAllCategoriesProds = (cat_id) => {
    ...
    setIsAllProds(false);
    console.log("categoriesProducts 12345:", categoriesProducts);
  };
  ...
  console.log("CART PRODUCTS 0-----", cart_products);
  ...
  const stickNavbar = () => {
    ...
    if (categoryAllProducts.length > 0) {
      if (window !== undefined) {
        ...
        console.log("WINDOW HEIGHT 222222222::::::::::::::: ", windowHeight);
        if (windowHeight > 420) {
      ...
      let windowHeight = window.scrollY;
      console.log("WINDOW HEIGHT 3333333::::::::::::::: ", windowHeight);
      if (windowHeight > 420) {
        setStickyClass("fixxed");
        setShowTopCat(true);
      } else {
  ...
  const getEccomSEO = async () => {
    const response = await axios.get(`${URL}/eccom_seo`, {
      headers: {
        Authorization: "Bearer " + localStorage.getItem("token123"),
      },
    });
    if (!response.error) {
      console.log("getEccomSEO - resp", response);
      setEccomSEO(response.data.data);

      const resp = response.data.data;
      let body_script = resp.body_script;

      var no_script = document.getElementsByTagName("noscript")[0];
      no_script.innerHTML = body_script;
      // }
    } else {
      console.log("getEccomSEO - response.error", response.error);
    }
  };
  ...
  return (
    <>
      ...
      <div>
        ...
        >
          <img src="/images/product_left_design.png" height={20} />
          <h1 id="trending_title">{trans("Our Trends")}</h1>
          <img src="/images/product_right_design.png" height={20} />
        </div>
        <Container fixed>
                ...
                <Col md={stickyClass === "fixxed" ? 10 : 12}>
                  <Slider
                    className={`categories_slider ${
                      stickyClass == "fixxed" && "dots_fixxed"
                    }`}
                    ref={slider}
                    {...settings}
                  >
                    <div className="testt">
                      ...
                      >
                        <CardContent
                          className={`${
                            isAllProds == true && "active_category"
                          } ${
                            isAllProds == true &&
                            stickyClass == "fixxed" &&
                          ...
                          style={{
                            ...
                            alignItems: "center",
                            paddingBottom: "0px",
                            // padding: stickyClass === "fixxed" ? "0px" : "16px",
                            overflow: stickyClass === "fixxed" && "unset",
                            zIndex: 99,
                            // boxShadow: stickyClass === "fixxed" && "none",
                            // borderRadius: stickyClass === "fixxed" && "0px",
                    ...
                    {allCategories != null
                      ...
                      : categoriesSkeleton.map((i, key) => (
                          <div className="testt" style={{ marginTop: "-8px" }}>
                            <Card
                              className={Styleclasses.CategorySlider}
                              key={key}
                              style={{
                            ...
                            <div className="image_box">
                              <div className="image_box_img">
                                <img
                                  ...
                                  src={`${process.env.PUBLIC_URL}/images/config_circle_light.png`}
                                  alt=""
                                />
                                <div className="box_image_overlay">
                                  <img
                                    src={`${process.env.PUBLIC_URL}/images/cart_coloured.png`}
                                    alt=""
                                  />
                                </div>
                              </div>
                          ...
                          >
                            {cart_products.length !== 0 &&
                              cart_products.map((product) => {
                                return (
                                  <ProdCartListItem product_info={product} />
                                );
                              })}
                          </div>
                        )}
