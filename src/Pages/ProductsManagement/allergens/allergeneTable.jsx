...
const AllergeneTable = (props) => {
  ...
  console.log("allergensList", allergenes)
    ...
    if (name === "alertDanger") {
      SweetAlert.fire({
        ...
        if (result.value) {
          ...
          dispatch(DeleteAllergenAction(id))
          SweetAlert.fire({
            icon: "success",
            title: trans("Deleted"),
  ...
  const bulkDeleteApiCall = (bulkIds) => {
    dispatch(AllergensBulkDeleteAction(bulkIds))
  };
        ...
        <Card>
          ...
          <CardBody>
            <DataTable
              ...
              isLoading={allergenes.allergensList.length == 0 ? true : false}
            />
          </CardBody>
...
export default translate(AllergeneTable);
