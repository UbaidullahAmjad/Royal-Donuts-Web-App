...
const SupplierCategories = (props) => {
  ...
  console.log("ingredientCategoriesList", supplierCatList)
    ...
    if (name === "alertDanger") {
      SweetAlert.fire({
        ...
        if (result.value) {
          ...
          dispatch(DeleteIngredientsCategoriesAction(id))
          SweetAlert.fire({
            icon: "success",
            title: trans("Deleted"),
  ...
  const bulkDeleteApiCall = (bulkIds) => {
    dispatch(IngredientsCategoriesBulkDeleteAction(bulkIds))
  };
        ...
        return (
          <div>
            {(role == "SuperAdmin" || permissions.match("edit") != null) && (
              <Link
                to={
                  "/supplier/categories/edit/" + cellValues.id + "/" + "RD"
                }
              >
          ...
          <Card>
            ...
            <CardBody>
              <DataGrid
                ...
                isLoading={supplierCatList.ingredientCategoriesList.length == 0 ? true : false}
              />
            </CardBody>
            <Modal
              isOpen={openCSV}
