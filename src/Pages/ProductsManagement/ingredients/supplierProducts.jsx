...
const SupplierProducts = (props) => {
  ...
  console.log("supplierProList", supplierProList)
  ...
  const handleOnClickSelectionModelDeleteButton = (bulkIds) => {
    SweetAlert.fire({
      ...
      if (result.value) {
        ...
        dispatch(IngredientsBulkDeleteAction(bulkIds));
      }
    });
  };
  ...
  const handleIsActive = (id, message) => {
    ...
    //   },
    //   url: `${URL}/supplier_product/status/${id}`,
    //   // data: "",
    // }).then((response) => {
    ...
    //       const response = await axios.get(`${URL}/supplier_product`, {
    //         headers: {
    //           Authorization: "Bearer " + localStorage.getItem("token123"),
    //         },
    //       });
        ...
        return (
          <div>
            {(role == "SuperAdmin" || permissions.match("edit") != null) && (
              <Link
                className=""
                to={"/supplier/products/edit/" + cellValues.id + "/" + "RD"}
              >
          ...
          <Card>
            ...
            <CardBody>
              <DataGrid
                ...
                isLoading={supplierProList.ingredientsList.length == 0 ? true : false}
              />
            </CardBody>
            <Modal
              isOpen={openCSV}
...
export default translate(SupplierProducts);
