...
import { InvoiceDataByIdAction } from "../../../redux/Pages/SupplierCustomer/CustomerOrders/CustomerInvoice"

import "./supplierOrderSummary.css";
...
const SupplierOrderSummary = (props) => {
  ...
  const getInvoiceDataById = async () => {
    ...
    const InvoiceDataByIdResponse = await InvoiceDataByIdAction(id)
    // console.log("InvoiceDataByIdResponse", InvoiceDataByIdResponse)
    if (InvoiceDataByIdResponse?.data?.success == true) {
      setInvoiceOrderData(InvoiceDataByIdResponse?.data?.order);
      setInvoiceOrderItems(InvoiceDataByIdResponse?.data?.order_items);
      setEmail(InvoiceDataByIdResponse?.data?.email);
      setUser(InvoiceDataByIdResponse?.data?.user);
    } else if (InvoiceDataByIdResponse?.data?.error == true) {
      toast.error(trans(InvoiceDataByIdResponse?.data?.message ?? "failed"));
    } else {
                                ...
                                <tbody>
                                  ...
                                  <tr>
                                    ...
                                    <td className="payment digits">
                                      {InvoiceOrderData != null ? (
                                        <p className="itemtext digits">
                                          {splitNumber(InvoiceOrderData.total.toFixed(2))} €
                                        </p>
                                      ) : (
                                        "0 €"
                                      )}
