...
import { URL } from "../env";
import logo from "../assets/images/logo/royal_donuts_logo.png";
...
const Logins = (props) => {
  ...
  const [PasswordRemember, setPasswordRemember] = useState(
    localStorage.getItem("passwordremember")
  );
  ...
  console.log(
    "PasswordRemember-localStorage.getItem(passwordremember)",
    localStorage.getItem("passwordremember")
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      ...
      localStorage.getItem("passwordremember") != undefined &&
        localStorage.getItem("passwordremember") != null
        ? localStorage.getItem("passwordremember")
        : false
    );
    setPasswordRemember(
      PasswordRemember != undefined && PasswordRemember != null
        ? PasswordRemember
        : false
    );
  ...
  const loginWithJwt = async (email, password) => {
    setLoading(true);
    const baseUrl = `${URL}/signin-pagee`;
    axios
      .post(baseUrl, {
        email,
        password,
      ...
      .then((user) => {
        ...
        if ((JSON.parse(PasswordRemember) == false) || (localStorage.getItem("passwordremember") == null)) {
          console.log("signin-axios_then-PasswordRemember_if", PasswordRemember);
          setTokenExpiryTime(expiryDate);
        } else {
          // localStorage.setItem("token_expiry_time", "null");
          console.log("signin-axios_then-PasswordRemember_else", PasswordRemember);
      ...
      .catch((err) => {
        console.log("signin-axios_catch");
        setLoading(false);
    ...
    if (isTokenAvailableState == true) {
      if (PasswordRemember && PasswordRemember == true) {
        if (atob(localStorage.getItem("role")) == "Supplier_Customer") {
          // <Navigate to={`/suppliers/customer/products/list`} replace />;
          navigate(`/suppliers/customer/products/list`);
        } else {
                      ...
                      <Form className="theme-form">
                        ...
                        <div className="password_remember_box">
                          ...
                          <div className="form-group ml-3 mb-0">
                            <Link
                              className="link"
                              to={`${process.env.PUBLIC_URL}/email/verification`}
