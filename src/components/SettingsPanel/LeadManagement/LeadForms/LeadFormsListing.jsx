...
const LeadFormsListing = (props) => {
  ...
  const Displayalert = (name, id) => {
    if (name === "alertDanger") {
      SweetAlert.fire({
        ...
        if (result.value) {
          dispatch(DeleteLeadFormAction(id))
          // DeleteItem(id);
          // SweetAlert.fire({
          //   icon: "success",
          //   title: trans("Deleted!"),
  ...
  const columns = [
    { field: "index", headerName: "#", flex: 0.6, minWidth: 100 },
    { field: "name", headerName: trans("First Name"), flex: 2, minWidth: 240 },
    {
      field: "action",
  ...
  return (
    <Fragment>
      ...
      <DataTable
        ...
        isLoading={allLeads.leadFormsList.length == 0 ? true : false}
      />
    </Fragment>
  );
};

export default translate(LeadFormsListing);
